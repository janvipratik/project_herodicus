...
class VideoDatatable extends DataTable
{
    ...
    public function dataTable($query)
    {
        return datatables()
            ...
            ->addColumn('user_name', function ($row) {
                return isset($row->user) ? $row->user->username : '-';
            })
    ...
    public function Status($row)
    {
        ...
        <option value='0' " . ($selectedValue == 0 ? "selected" : "") . ">Rejected</option>
        <option value='1' " . ($selectedValue == 1 ? "selected" : "") . ">Approve</option>";
        $status .= "
        </select>
        </div>
        ";
