...
class SubscriptionDatatable extends DataTable
{
    ...
    public function dataTable($query)
    {
        return datatables()
            ...
            ->addColumn('modelType', function ($row) {
                $model = '';
                if ($row->model_type == 0) {
                    $model = 'Year';
                } elseif($row->model_type == 1) {
                    $model = 'Month';
                }
                return $model;
            })
            ->rawColumns(['action', 'status', 'modelType']);
    }
    public function checkrights($row)
    {
        $menu = '';
        $editurl = route('subscriptions.edit', ['subscription' => $row->id]);
    ...
    public function query(SubscriptionDatatable $model)
    {
        ...
        if (!request()->has('order')) {
            $models->orderBy('subscriptions.updated_at', 'DESC');
        }
